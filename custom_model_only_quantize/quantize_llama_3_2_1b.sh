...
for b in "${BLOCK_SIZES[@]}"; do
  ...
  python "$(dirname "$0")/block-floating-point/bfp_quantize.py" \
    --model "${MODEL}" \
    --block-size "${b}" \
...
for b in "${BLOCK_SIZES[@]}"; do
  ...
  python "$(dirname "$0")/fix-precision-awq/fix_precision_awq.py" \
    --model "${MODEL}" \
    --dataset "${DATASET}" \
...
for b in "${BLOCK_SIZES[@]}"; do
  ...
  python "$(dirname "$0")/mix-precision-awq/mix_precision_awq.py" \
    --model "${MODEL}" \
    --dataset "${DATASET}" \
    --dataset-config "${DATASET_CONFIG}" \
    --num-samples "${NUM_SAMPLES}" \
