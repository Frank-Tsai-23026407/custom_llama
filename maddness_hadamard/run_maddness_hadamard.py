...
import numpy as np
from maddness_hadamard import MaddnessHadamardLayer, MaddnessHadamardTrainer
import copy
import sys
...
def main():
    ...
    results_file = "maddness_hadamard_evaluation_results.txt"
    with open(results_file, "w") as f:
        f.write("Depth | Perplexity | HellaSwag Acc\n")
        f.write("---|---|---\n")
    
    for depth in tree_depths:
        print(f"\n==========================================")
        print(f"Evaluating Tree Depth: {depth}")
        print(f"==========================================")
        
        # Reload model fresh for each depth
        print("Reloading base model...")
        model = AutoModelForCausalLM.from_pretrained(model_path, torch_dtype=torch.float16, device_map="auto")
        
        maddness_model_path = f"maddness_hadamard_tinyllama_{NUM_SUBSPACES}_{depth}.pt"
        ...
        if os.path.exists(maddness_model_path):
            ...
            for name, original_layer in tqdm(model.named_modules(), desc="Replacing Layers (Loading)"):
                 ...
                 maddness_layer = MaddnessHadamardLayer(original_layer, subspace_dim=original_layer.in_features // NUM_SUBSPACES, tree_depth=depth)
            ...
            for name, original_layer in tqdm(modules_to_replace.items(), desc="Replacing Layers"):
                ...
                trainer = MaddnessHadamardTrainer(original_layer.in_features, subspace_dim=original_layer.in_features // NUM_SUBSPACES, tree_depth=depth)
                W = original_layer.weight.data.to(torch.float32)
                    
                split_indices, split_thresholds, lookup_table, _, mse, max_diff = trainer.train(X_calib.to(model.device), W.to(model.device))
        ...
        with open(results_file, "a") as f:
            f.write(f"{depth} | {ppl:.2f} | {hellaswag_acc:.4f}\n")
