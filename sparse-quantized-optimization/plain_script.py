...
class ComputationTracker:
    ...
    def reset(self):
        self.total_ops = 0
        self.high_prec_ops = 0
        self.low_prec_ops = 0
        
    def update(self, high_prec=0, low_prec=0):
        self.high_prec_ops += high_prec
        self.low_prec_ops += low_prec
        self.total_ops += (high_prec + low_prec)
        
    def get_stats(self):
        return {
            "total_ops": self.total_ops,
            "high_prec_ops": self.high_prec_ops,
            "low_prec_ops": self.low_prec_ops,
            "high_prec_ratio": self.high_prec_ops / self.total_ops if self.total_ops > 0 else 0
        }

tracker = ComputationTracker()

# Helper function for dynamic mix precision matrix multiplication
def dynamic_mix_precision_matmul(x, w_quant, w_bf16, ratio=0.0):
    # x: (..., in_dim)
    # w_quant: (out_dim, in_dim)
    # w_bf16: (out_dim, in_dim)
    
    # Calculate operations
    # Output shape: (..., out_dim)
    # Ops = batch_size * seq_len * in_dim * out_dim
    # x shape: (batch, seq, in_dim) or (batch, in_dim) if seq=1 squeezed? 
    # Usually x is (..., in_dim). 
    # Total elements in output = product(x.shape[:-1]) * w_quant.shape[0]
    # Ops per element = w_quant.shape[1] (dot product size)
    # Total ops = product(x.shape[:-1]) * w_quant.shape[0] * w_quant.shape[1]
    
    batch_tokens = x.numel() // x.shape[-1]
    out_dim = w_quant.shape[0]
    in_dim = w_quant.shape[1]
    total_matmul_ops = batch_tokens * out_dim * in_dim
    
    if ratio <= 0.0 or w_bf16 is None:
        tracker.update(low_prec=total_matmul_ops)
        return torch.matmul(x, w_quant.t())
        
    # Base calculation with quantized weights (Low Precision)
    base_out = torch.matmul(x, w_quant.t())
    tracker.update(low_prec=total_matmul_ops)
    ...
    # Find top-k indices
    topk_indices = torch.topk(x_abs, k, dim=-1).indices
    mask = torch.zeros_like(x)
    mask.scatter_(-1, topk_indices, 1.0)
    
    # Calculate correction: (x * mask) @ (w_bf16 - w_quant).T
    # This is effectively a sparse matmul, but implemented as dense for simplicity here.
    # The "effective" high precision ops are only for the selected channels.
    # Ops = batch_tokens * out_dim * k
    high_prec_ops = batch_tokens * out_dim * k
    tracker.update(high_prec=high_prec_ops)
    
    w_diff = w_bf16 - w_quant
    correction = torch.matmul(x * mask, w_diff.t())
...
def mqa_rope(i, wq, wk, wv, wo, num_heads, num_kv_heads, get_attn_scores=False, precision_policy: Optional[PrecisionPolicy]=None,
             wq_bf16=None, wk_bf16=None, wv_bf16=None, wo_bf16=None, dynamic_mix_ratio=0.0):
    ...
    q_proj = dynamic_mix_precision_matmul(i_mm, wq.to(matmul_dtype), wq_bf16.to(matmul_dtype) if wq_bf16 is not None else None, dynamic_mix_ratio)
    k_proj = dynamic_mix_precision_matmul(i_mm, wk.to(matmul_dtype), wk_bf16.to(matmul_dtype) if wk_bf16 is not None else None, dynamic_mix_ratio)
    v_proj = dynamic_mix_precision_matmul(i_mm, wv.to(matmul_dtype), wv_bf16.to(matmul_dtype) if wv_bf16 is not None else None, dynamic_mix_ratio)
    ...
    # Ops = B * H * S * S * D
    # This is usually done in high precision (bf16/fp32) or whatever matmul_dtype is.
    # We'll count this as "low_prec" if it's not explicitly the dynamic mix part, 
    # OR we can track it separately. The user asked for "ratio of over all matmul".
    # Since we don't apply dynamic mix to Q*K, we count it as standard ops.
    # If matmul_dtype is fp16/bf16, it's technically high precision compared to 4-bit weights,
    # but in the context of "IF x W" vs "others", this is "others".
    # Let's count it as "standard" (low_prec bucket for now, or maybe we need a separate bucket?)
    # The user asked for "full precision ratio of the IF x W, and the ratio of over all matmul".
    # So we should probably count everything.
    # Let's assume "low_prec" in tracker is "standard/quantized" and "high_prec" is "extra high precision work".
    # Wait, Q*K is typically high precision (fp16) in standard LLM inference, while W is quantized.
    # If we want "ratio of IF x W (high part)", we have that.
    # If we want "ratio of overall matmul", we need total matmul ops.
    
    # Q @ K.T
    q_ops = batch_size * num_q_heads * seq_len * seq_len * head_dim
    tracker.update(low_prec=q_ops) # Counting as standard op
    ...
    # Ops = B * H * S * D * S
    v_ops = batch_size * num_q_heads * seq_len * head_dim * seq_len
    tracker.update(low_prec=v_ops) # Counting as standard op
    ...
    o = dynamic_mix_precision_matmul(attn_output, wo.to(matmul_dtype), wo_bf16.to(matmul_dtype) if wo_bf16 is not None else None, dynamic_mix_ratio) # Final output
    if get_attn_scores:
...
def ffn_SwiGLU(i, w_gate, w_up, w_down, precision_policy: Optional[PrecisionPolicy]=None,
               w_gate_bf16=None, w_up_bf16=None, w_down_bf16=None, dynamic_mix_ratio=0.0):
    # i: input tensor (batch_size, seq_len, model_dim)
    ...
    gate = dynamic_mix_precision_matmul(i_ffn, w_gate.to(ffn_dtype), w_gate_bf16.to(ffn_dtype) if w_gate_bf16 is not None else None, dynamic_mix_ratio)
    up = dynamic_mix_precision_matmul(i_ffn, w_up.to(ffn_dtype), w_up_bf16.to(ffn_dtype) if w_up_bf16 is not None else None, dynamic_mix_ratio)
    ...
    output = dynamic_mix_precision_matmul(hidden_states, w_down.to(ffn_dtype), w_down_bf16.to(ffn_dtype) if w_down_bf16 is not None else None, dynamic_mix_ratio)
...
def transformer_block(x, params, num_heads, num_kv_heads, get_attention_scores=False):
    ...
    attn_output, attn_scores = mqa_rope(
        ...
        wq_bf16=params.get('wq_bf16'), wk_bf16=params.get('wk_bf16'), wv_bf16=params.get('wv_bf16'), wo_bf16=params.get('wo_bf16'),
        dynamic_mix_ratio=params.get('dynamic_mix_ratio', 0.0)
    )
    ...
    ffn_output = ffn_SwiGLU(
        ...
        precision_policy=params.get('precision_policy'),
        w_gate_bf16=params.get('w_gate_bf16'), w_up_bf16=params.get('w_up_bf16'), w_down_bf16=params.get('w_down_bf16'),
        dynamic_mix_ratio=params.get('dynamic_mix_ratio', 0.0)
    )
...
class transfomer_block_with_kv_cache:
    ...
    def forward(self, x, params):
        ...
        # Projections
        q_proj = dynamic_mix_precision_matmul(x_mm, params['wq'].to(mm_dtype), params.get('wq_bf16').to(mm_dtype) if params.get('wq_bf16') is not None else None, params.get('dynamic_mix_ratio', 0.0))
        k_proj = dynamic_mix_precision_matmul(x_mm, params['wk'].to(mm_dtype), params.get('wk_bf16').to(mm_dtype) if params.get('wk_bf16') is not None else None, params.get('dynamic_mix_ratio', 0.0))
        v_proj = dynamic_mix_precision_matmul(x_mm, params['wv'].to(mm_dtype), params.get('wv_bf16').to(mm_dtype) if params.get('wv_bf16') is not None else None, params.get('dynamic_mix_ratio', 0.0))
        # Reshape heads
        ...
        attn_output = attn_output.transpose(1, 2).contiguous().view(batch_size, seq_len, model_dim)
        o = dynamic_mix_precision_matmul(attn_output.to(mm_dtype), params['wo'].to(mm_dtype), params.get('wo_bf16').to(mm_dtype) if params.get('wo_bf16') is not None else None, params.get('dynamic_mix_ratio', 0.0)).to(self.device)
        x = x + o
        # LayerNorm 2
