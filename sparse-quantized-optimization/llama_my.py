...
class LlamaMyModel:
    ...
    def __init__(self, model_name="TinyLlama/TinyLlama_v1.1", device='cpu', stop_criteria=None, dtype=torch.float32,
        ...
        for layer_idx in range(self.num_layers):
            ...
            if apply_bfp:
                def get_m_bits(module_suffix):
                    full_name = f"model.layers.{layer_idx}.{module_suffix}"
                    if mixed_precision_config and full_name in mixed_precision_config:
                        return mixed_precision_config[full_name]
                    return bfp_mantissa_bits
                ...
                layer.self_attn.q_proj.weight.data = block_floating_point_quantize(layer.self_attn.q_proj.weight.data, block_size=bfp_block_size, mantissa_bits=get_m_bits('self_attn.q_proj'))
                layer.self_attn.k_proj.weight.data = block_floating_point_quantize(layer.self_attn.k_proj.weight.data, block_size=bfp_block_size, mantissa_bits=get_m_bits('self_attn.k_proj'))
                layer.self_attn.v_proj.weight.data = block_floating_point_quantize(layer.self_attn.v_proj.weight.data, block_size=bfp_block_size, mantissa_bits=get_m_bits('self_attn.v_proj'))
                layer.self_attn.o_proj.weight.data = block_floating_point_quantize(layer.self_attn.o_proj.weight.data, block_size=bfp_block_size, mantissa_bits=get_m_bits('self_attn.o_proj'))
                ...
                layer.mlp.gate_proj.weight.data = block_floating_point_quantize(layer.mlp.gate_proj.weight.data, block_size=bfp_block_size, mantissa_bits=get_m_bits('mlp.gate_proj'))
                layer.mlp.up_proj.weight.data = block_floating_point_quantize(layer.mlp.up_proj.weight.data, block_size=bfp_block_size, mantissa_bits=get_m_bits('mlp.up_proj'))
                layer.mlp.down_proj.weight.data = block_floating_point_quantize(layer.mlp.down_proj.weight.data, block_size=bfp_block_size, mantissa_bits=get_m_bits('mlp.down_proj'))
            params = {
    ...
    def apply_magnitude_pruning(self, sparsity_ratio):
        """Applies unstructured magnitude pruning to all linear layers in the model.

        Args:
            sparsity_ratio (float): The percentage of weights to prune (0.0 to 1.0).
        """
        print(f"Applying magnitude pruning with sparsity ratio: {sparsity_ratio}")
        
        # Gather all linear layers (including q, k, v, o, gate, up, down)
        # We prune based on the global threshold or layer-wise? 
        # The prompt implies "naive 50% magnitude pruning", usually layer-wise or global.
        # Let's do layer-wise for simplicity and common practice in simple baselines, 
        # or we can do it per-tensor.
        # Let's do per-tensor (weight matrix) pruning as it's the most "naive" local approach.
        ...
        for name, module in self.model.named_modules():
            if isinstance(module, torch.nn.Linear):
                ...
                if k > 0:
                    ...
                    # Verify sparsity
                    current_sparsity = 1.0 - (module.weight.data != 0).float().mean().item()
                    # print(f"Layer {name} sparsity: {current_sparsity:.4f}")
        print("Magnitude pruning applied.")

    def apply_wanda_pruning(self, sparsity_ratio, activation_scales):
        """Applies Wanda (Pruning by Weights and activations) to linear layers.

        Args:
            sparsity_ratio (float): The percentage of weights to prune.
            activation_scales (dict): A dictionary mapping layer names to activation scale tensors.
                                      Keys should match module names (e.g., 'model.layers.0.self_attn.q_proj').
        """
        print(f"Applying Wanda pruning with sparsity ratio: {sparsity_ratio}")
        
        for name, module in self.model.named_modules():
            if isinstance(module, torch.nn.Linear):
                if name not in activation_scales:
                    # print(f"Warning: No activation scale found for {name}, skipping Wanda for this layer.")
                    continue
                
                # Check if we should skip this layer (High Precision)
                if self.mixed_precision_config and name in self.mixed_precision_config:
                    bits = self.mixed_precision_config[name]
                    if bits > 5: # Assuming 5 is the "base" low precision
                        # print(f"Skipping pruning for high-precision layer {name} (bits={bits})")
                        continue

                scaler = activation_scales[name]
                
                # Wanda metric: |W| * ||X||
                # scaler is ||X|| (mean absolute value of input activations)
                # Ensure scaler is on the same device and has correct shape
                # scaler shape should be (1, in_features) to broadcast with weight (out_features, in_features)
                
                if scaler.dim() == 1:
                    scaler = scaler.unsqueeze(0)
                
                scaler = scaler.to(module.weight.device).to(module.weight.dtype)
                
                # Calculate metric
                # W * scale
                # We want to prune weights that have small magnitude * activation magnitude
                W_metric = torch.abs(module.weight.data) * scaler
                
                # Determine threshold
                k = int(module.weight.numel() * sparsity_ratio)
                if k > 0:
                    threshold = torch.kthvalue(W_metric.flatten(), k).values
