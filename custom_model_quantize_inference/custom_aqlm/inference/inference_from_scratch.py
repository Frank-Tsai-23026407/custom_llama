...
def main():
    ...
    if args.quantized_path.endswith('.pt'):
        print(f"Loading full model checkpoint from {args.quantized_path}...")
        # We need to ensure custom classes are available. 
        # Since we added current_dir to sys.path, 'src' should be resolvable 
        # IF the checkpoint was saved with 'src' in the path.
        # It might also need 'aqlm' in path if saved from outside.
        
        try:
            model = torch.load(args.quantized_path, map_location=args.device)
            # Handle weights_only=False default change in newer torch if needed, 
            # but usually torch.load handles it unless specific arg passed? 
            # The user error showed it defaulted to True.
        except Exception:
             # Retry with weights_only=False if available
             model = torch.load(args.quantized_path, map_location=args.device, weights_only=False)
