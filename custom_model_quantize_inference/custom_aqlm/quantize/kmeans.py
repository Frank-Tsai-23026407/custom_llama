...
from custom_aqlm.utils.utils import maybe_script
...
def fit_kmeans(
    ...
    :param devices: (DEPRECATED: unused)
    :return: (clusters float[k, dim], data_indices int[nsamples], reconstructed_data: float[nsamples, dim])
    """
    if devices is not None and len(devices) > 1:
        print("Warning: fit_kmeans received multiple devices, but parallel K-means is disabled.")
    ...
    # Single device logic
    nearest_indices = torch.empty(len(data), dtype=torch.int64, device=device)
    ...
    for i in range(max_iter):
        ...
        new_clusters = (
            clusters.clone()
            .index_reduce_(dim=0, index=nearest_indices, source=data, reduce="mean", include_self=False)
        )
...
def find_nearest_cluster(data, clusters, block_size_vals: int = 2**30, devices: Optional[List[torch.device]] = None):
    ...
    if devices is not None and len(devices) > 1:
        print("Warning: find_nearest_cluster received multiple devices, but parallel is disabled.")

    device = data.device
    # Ensure simplified logic similar to fit_kmeans
