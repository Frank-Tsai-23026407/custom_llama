...
from custom_aqlm.utils.utils import _dequantize_weight, maybe_script
    ...
    *,
    ...
    for input_group_index in _make_range(num_in_groups):
        for codebook_index in _make_range(num_codebooks):
            # --- LOOP STRUCTURE ---
            # We iterate over every input group (block of input features) and every codebook.
            # At each step, we try to find a better code for the current (input_group, codebook) tuple.
            
            ### Part 1: Compute losses for every candidate code.
            # We want to change the code at [input_group_index, codebook_index].
            # There are 'codebook_size' (e.g. 256) possible new codes.
            # We compute the resulting MSE for ALL of them in parallel.
            # This is done for all output features simultaneously (vectorized).
            ...
            )  # Result shape: [current beam_size, k_best, num_out_groups]
