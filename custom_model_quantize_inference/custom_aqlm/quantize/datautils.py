...
def get_wikitext2(nsamples, seqlen, tokenizer, eval_mode=False):
    if not eval_mode:
        ...
        # Tokenize examples individually to be much faster than tokenizing one giant string
        print("Tokenizing training data...")
        trainloader = []
        
        # We need a large enough pool of tokens to sample from
        all_input_ids = []
        for i in range(len(traindata)):
            if len(traindata[i]["text"]) > 5: # skip empty lines
                ids = tokenizer.encode(traindata[i]["text"], add_special_tokens=False)
                all_input_ids.extend(ids)
            if len(all_input_ids) > (nsamples + 50) * seqlen: # break early if we have enough
                break
        
        all_input_ids = torch.tensor([all_input_ids])
        
        loop_samples = nsamples if nsamples is not None else 0
        for _ in range(loop_samples):
            if all_input_ids.shape[1] <= seqlen:
                break
            i = random.randint(0, all_input_ids.shape[1] - seqlen - 1)
            j = i + seqlen
            inp = all_input_ids[:, i:j]
            trainloader.append(inp)
        return trainloader
    else:
