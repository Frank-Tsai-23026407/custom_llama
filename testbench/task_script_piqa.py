...
def main():
    ...
    # Load PIQA dataset from the Hub (using modern parquet-based format)
    dataset = load_dataset("gimmaru/piqa", split="validation", trust_remote_code=False)
    ...
    for batch in tqdm(data_loader, desc="Evaluating PIQA", disable=args.disable_tqdm, file=tqdm_file):
        ...
        with torch.no_grad():
            ...
            if total_questions <= 5:
                ...
                for idx, ch in enumerate(choices):
                    print(f"{idx}. {ch} (Total NLL: {choice_total_nll[idx]:.2f}, Mean NLL: {choice_mean_nll[idx]:.2f})")
                print(f"True Answer: {true_label}")
                print(f"Predicted Answer (acc): {predicted_choice_idx}")
                print(f"Predicted Answer (acc_norm): {predicted_choice_idx_norm}")
                print("-" * 30)
