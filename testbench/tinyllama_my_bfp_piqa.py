...
def main():
    parser = argparse.ArgumentParser(description="Evaluate TinyLlama on PIQA with optional BFP quantization.")
    ...
    # Load PIQA dataset
    # Explicitly set a cache directory to avoid conflicts with local files.
    # This forces the library to download from the Hugging Face Hub.
    cache_dir = os.path.join(os.path.expanduser("~"), ".cache", "huggingface", "datasets", "piqa_explicit")
    if os.path.exists(cache_dir):
        import shutil
        shutil.rmtree(cache_dir) # Clean up cache to ensure a fresh download
    ...
    for batch in tqdm(data_loader, desc="Evaluating PIQA"):
        ...
        with torch.no_grad():
            ...
            if total_questions <= 5:
                ...
                print(f"Predicted Answer: {predicted_choice_idx}")
                print("-" * 30)

    accuracy = correct_predictions / total_questions
    print(f"\n--- PIQA Evaluation Results ---")
